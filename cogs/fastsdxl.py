...
class Fastsdxl(commands.Cog):
    ...
    async def fastsdxl_command(self, ctx: commands.Context, *, promt: str):
        async with aiohttp.ClientSession() as session:
            response = await session.post(url="https://fal.run/fal-ai/fast-lightning-sdxl",
    ...
    async def move_channel(self, ctx, promt: str, number_images: int = 1, negative: str = ""):
        if number_images > 4:
            await ctx.respond("Слишком много изображений, макксимум `4` изображения в одном запросе", ephimeral=True)
            return
        ...
        response = await response.json()
        if ctx.channel.nsfw:
            await ctx.respond(" ".join([response["images"][x]["url"] for x in range(len(response["images"]))]))
            return
        if not any(response["has_nsfw_concepts"]):
            await ctx.respond(" ".join([response["images"][x]["url"] for x in range(len(response["images"]))]))
